...
徽标又叫标识。序列标志图是显示序列保守区域的共识序列、每个位置上各个氨基酸或核苷酸出现的频率以及各个位点上的序列信息量的一种可视化方法。
...
在WebLogo程序（Crooks et al.，2004）绘制的序列标识图里，每个字符按其化学性质着色。对于DNA/RNA序列，核苷酸A、G、C、T（U）的默认色分别是绿、橘、蓝、红。对于蛋白质序列，带极性侧链的氨基酸（G、S、T、Y、C、Q、N）着绿色；碱性氨基酸（K、R、H）着蓝色；酸性氨基酸（D、E）着红色；疏水氨基酸（A、V、L、I、P、W、F、M）着黑色。此外，用户也可以根据自己的需求配色。
...
绘制box plot时，可以使用 \href{http://boxplot.tyerslab.com/}{BoxPlotR}、\href{https://plot.ly/}{Plotly}、\href{https://yiplab.cse.cuhk.edu.hk/ecplot/}{ECplot}、\href{https://usegalaxy.org/}{Galaxy}（“Graph/Display Data”工具集中的Boxplot）、R等。
...
\begin{enumerate}
  ...
  \item “ Mean ± SEM ” or “ Mean (SD)”? (2010), 42(5), 5–6.
...
\begin{enumerate}
  ...
  \item Hillman-Jackson, J., Clements, D., Blankenberg, D., Taylor, J., \& Nekrutenko, A. (2012). Using Galaxy to perform large-scale interactive data analyses. Current protocols in bioinformatics (Vol. Chapter 10).
