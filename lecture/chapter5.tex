...
如果要用一种方法把上面四条序列用另外一种方法表示出来，作为一名程序员，我首先想到的是正则表达式（regular
expression）——\verb|TA[YF]M[IML][AT][ED]M[ML]K|。在做查找的时候regular expression无疑能将所有满足要求的序列都找出来，但是它不直观，也不能反映出现频率。而Sequence logo就能让这些信息直观的显示出来（如下图所示）。
\begin{center}
