...
sub mcdonald_kreitman {
    ...
    for my $codon ( @marker_names ) { # Puzzle: returns SITEs instead of CODON
	my (%codonvals);
	my %all_alleles;
	for my $t ( @vt ) {
	    my $outcount = 1;
	    for my $ind ( $vals{$t}->get_Individuals() ) {
	...
	if( keys %{$codonvals{all}} <= 1 ) {
		...
		if( scalar @ingroup_codons == 1 ) { 
			...
			warn("more than 2 ingroup codons (@ingroup_codons)\n");	
                ...
                next if $ingroup_codons[0] eq '---' or $ingroup_codons[1] eq '---';
